...
# Modified by DarkC343 from github. for more information see: https://github.com/DarkC343/fracn
# For original version see: http://fractional-divider.tripod.com
...
use vars qw($base_name $vhdl_name $verilog_name $verilog_tb_name);
...
# open verilog test bench file
print "opening $verilog_tb_name\n" if ($verbose);
rename $verilog_tb_name, "$verilog_tb_name.bak";
open (VERILOG_TB, ">$verilog_tb_name") or die "Can't open Verilog file $verilog_tb_name: $!";
...
sub verilog_tb {
    foreach my $line (@_) {
        print VERILOG_TB ($tabstop > 0) ? expand($line) : $line;
    }
}
...
# write a comment to verilog test bech file
sub comment_tb
{
    foreach my $line (@_) {
        if ( $line =~ m/^(\t+)(.*)$/s ) {
...
# write a solid "separator" line to verilog test bench file
sub comment_line_tb
{
    verilog_tb "/" x 80 . "\n";
}
...
### start verilog testbech

comment_line_tb;
comment_tb " $verilog_tb_name\n";
comment_tb " D.O.B.       : 7/14/19\n";
comment_tb " Author       : DarkC343\n";
comment_tb " Description  : Test bench for fracn.v, file generated by fracn09.pl\n";
comment_tb "                (This can be run from the TCL script run_modelsim09.do)\n";
comment_line_tb;
verilog_tb "\n";
verilog_tb "`timescale 1 fs / 1 fs\n";
verilog_tb "\n";
verilog_tb "module tb_fracn09;\n";
verilog_tb "\t`define test_ClockDivider 1'b0\t\t// try another type of divider from Walter Baeck\n";
verilog_tb "\t`define fin $fin\t\t\t\t\t// Hz, input frequency\n";
verilog_tb "\t`define fout $fout\t\t\t\t// Hz, desired output frequency\n";
verilog_tb "\t`define time_of_report 100 * 1000000\n";
verilog_tb "\t`define use_output_50 1'b0\t\t\t// select which output of dut to use\n";
verilog_tb "\t// for frequency and jitter tests\n";
verilog_tb "\t// The next four generics are passed to the divider under test\n";
verilog_tb "\t`define use_phase_accumulator 1'b0\n";
verilog_tb "\t`define use_recursive_controller 1'b1\n";
verilog_tb "\t`define minimum_jitter 1'b0\n";
verilog_tb "\t`define improve_duty_cycle 1'b1\n";
verilog_tb "\n";
verilog_tb "\treg async_reset;\t\t// active high reset\n";
verilog_tb "\treg clock;\t\t\t\t// input clock\n";
verilog_tb "\treg clock_enable;\t\t// active high enable\n";
verilog_tb "\twire output_50;\t\t\t// approx 50% duty cycle\n";
verilog_tb "\twire output_pulse;\t\t// high for single clock\n";
verilog_tb "\n";
verilog_tb "\treal measured_frequency;\t// in Hz.\n";
verilog_tb "\treal measured_jitter;\n";
verilog_tb "\treal measured_duty_cycle_min;\n";
verilog_tb "\treal measured_duty_cycle_avg;\n";
verilog_tb "\treal measured_duty_cycle_max;\n";
verilog_tb "\n";
verilog_tb "\t`define clock_half_period (0.5e15/`fin) * 1\n";
verilog_tb "\t`define clock_period 2 * `clock_half_period\n";
verilog_tb "\n";
verilog_tb "\t`define ideal_ratio `fin/`fout\n";
verilog_tb "\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\t// Save the results to a file at a certain time (and repeat)\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\tparameter report_file_name = \"fracn.rpt\";\n";
verilog_tb "\tinteger f;\n";
verilog_tb "\tinitial\n";
verilog_tb "\t\tbegin\n";
verilog_tb "\t\t\tf = \$fopen(report_file_name, \"w\");\n";
verilog_tb "\t\t\tforever #(`time_of_report)\n";
verilog_tb "\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"%0d\", measured_frequency);\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"\\t\");\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"%0d\", ((measured_frequency - `fout) / `fout)*1.0); // frequency error\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"\\t\");\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"%0d\", measured_jitter);\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"\\t\");\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"%0d\", measured_duty_cycle_min*100.0);\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"/\");\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"%0d\", measured_duty_cycle_avg*100.0);\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"/\");\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"%0d\", measured_duty_cycle_max*100.0);\n";
verilog_tb "\t\t\t\t\t\$fwrite(f, \"\\n\");\n";
verilog_tb "\t\t\t\tend\n";
verilog_tb "\t\tend\n";
verilog_tb "\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\t// Instantiate the divider under test\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\tgenerate\n";
verilog_tb "\t\tif (~`test_ClockDivider)\n";
verilog_tb "\t\tbegin : select_allan_divider\n";
verilog_tb "\t\tfracn dut(\n";
verilog_tb "\t\t\t.async_reset(async_reset),\n";
verilog_tb "\t\t\t.clock(clock),\n";
verilog_tb "\t\t\t.clock_enable(clock_enable),\n";
verilog_tb "\t\t\t.output_50(output_50),\n";
verilog_tb "\t\t\t.output_pulse(output_pulse)\n";
verilog_tb "\t\t);\n";
verilog_tb "\t\tend\n";
verilog_tb "   endgenerate\n";
verilog_tb "\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\t// Make the test clock signal\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\t";
verilog_tb "\t// clock generator\n";
verilog_tb "\tinitial\n";
verilog_tb "\t\tbegin: clock_generator\n";
verilog_tb "\t\t\tclock_enable <= 1'b1;\n";
verilog_tb "\t\t\tclock <= 1'b1;\n";
verilog_tb "\t\t\tforever #(`clock_half_period) clock <= ~clock;\n";
verilog_tb "\t\tend\n";
verilog_tb "\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\t// Make the test reset signal\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\n";
verilog_tb "\t// reset generator\n";
verilog_tb "\tinitial\n";
verilog_tb "\t\tbegin: reset_generator\n";
verilog_tb "\t\t\tasync_reset <= 1'b1;\n";
verilog_tb "\t\t\t#(`clock_half_period);\n";
verilog_tb "\t\t\tasync_reset <= 1'b0;\n";
verilog_tb "\t\tend\n";
verilog_tb "\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\t// Measure the output frequency and jitter\n";
verilog_tb "\t//\n";
verilog_tb "\t// The frequency measured will be correct in the long term,\n";
verilog_tb "\t// but in the short term, jitter will cause slight errors in measurement.\n";
verilog_tb "\t//\n";
verilog_tb "\t// Note that the method used will cause long term frequency errors to affect\n";
verilog_tb "\t// the jitter measurement.\n";
verilog_tb "\t//\n";
verilog_tb "\t// We measure either output_50 or output_pulse depending on\n";
verilog_tb "\t// the value of the use_output_50 generic.\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\tgenerate\n";
verilog_tb "\t\tif (~`use_output_50)\n";
verilog_tb "\t\t\tbegin : measure_pulse\n";
verilog_tb "\t\t\t\t\n";
verilog_tb "\t\t\t\talways @(posedge async_reset or posedge clock)\n";
verilog_tb "\t\t\t\t\tbegin: measure_frequency\n";
verilog_tb "\t\t\t\t\t\treal   num_outputs;\n";
verilog_tb "\t\t\t\t\t\treal   num_clocks;\n";
verilog_tb "\t\t\t\t\t\ttime      phase_error;\n";
verilog_tb "\t\t\t\t\t\ttime      max_phase_error;\n";
verilog_tb "\t\t\t\t\t\ttime      min_phase_error;\n";
verilog_tb "\t\t\t\t\t\treg waiting;\n";
verilog_tb "\t\t\t\t\t\tif (async_reset == 1'b1)\n";
verilog_tb "\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\tnum_outputs = 0;\n";
verilog_tb "\t\t\t\t\t\t\t\tnum_clocks = 0;\n";
verilog_tb "\t\t\t\t\t\t\t\tphase_error = 0;\n";
verilog_tb "\t\t\t\t\t\t\t\tmax_phase_error = -1 * 1000000000;\n";
verilog_tb "\t\t\t\t\t\t\t\tmin_phase_error = 1 * 1000000000;\n";
verilog_tb "\t\t\t\t\t\t\t\tmeasured_jitter <= 0;\n";
verilog_tb "\t\t\t\t\t\t\t\twaiting = 1'b1;\n";
verilog_tb "\t\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\t\telse \n";
verilog_tb "\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\tif (waiting)\n";
verilog_tb "\t\t\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t// don't take any measurements until after the first output pulse\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\tif (output_pulse == 1'b1)\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\twaiting = 1'b0;\n";
verilog_tb "\t\t\t\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\t\t\t\telse\n";
verilog_tb "\t\t\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\tnum_clocks = num_clocks + 1;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\tif (output_pulse == 1'b1)\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\tnum_outputs = num_outputs + 1;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\tmeasured_frequency <= ((num_outputs)/(num_clocks)) * `fin;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\tphase_error = (((num_outputs) * `ideal_ratio) - (num_clocks)) * `clock_period;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\tif (phase_error > max_phase_error)\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\t\tmax_phase_error = phase_error;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\t\tmeasured_jitter <= max_phase_error - min_phase_error;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\tif (phase_error < min_phase_error)\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\t\tmin_phase_error = phase_error;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\t\tmeasured_jitter <= max_phase_error - min_phase_error;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\t\t\t\t\tend\n";
...
verilog_tb "\t";
comment_line_tb;
verilog_tb "\t// Measure the min/avg/max duty cycle of the \"50%\" duty cycle output.\n";
verilog_tb "\t// Note that times are turned into integers by dividing them by another time,\n";
verilog_tb "\t// and if we use the (FAQ suggested) value of 1 fs, we will get overflow\n";
verilog_tb "\t// problems after 2.1 us.  Hence the division by half the clock period.\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\n";
verilog_tb "\talways @(posedge async_reset or posedge output_50 or negedge output_50)\n";
verilog_tb "\t\tbegin: measure_duty_cycle\n";
verilog_tb "\t\t\ttime      total_time_high;\n";
verilog_tb "\t\t\ttime      total_time_low;\n";
verilog_tb "\t\t\ttime      last_rising;\n";
verilog_tb "\t\t\ttime      last_falling;\n";
verilog_tb "\t\t\ttime      pw_high;\n";
verilog_tb "\t\t\ttime      pw_low;\n";
verilog_tb "\t\t\treal duty_cycle;\n";
verilog_tb "\t\t\treal duty_cycle_min;\n";
verilog_tb "\t\t\treal duty_cycle_max;\n";
verilog_tb "\t\t\tif (async_reset == 1'b1)\n";
verilog_tb "\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\tmeasured_duty_cycle_min <= 0.0;\n";
verilog_tb "\t\t\t\t\tmeasured_duty_cycle_avg <= 0.0;\n";
verilog_tb "\t\t\t\t\tmeasured_duty_cycle_max <= 0.0;\n";
verilog_tb "\t\t\t\t\tduty_cycle_min = 1.0;\n";
verilog_tb "\t\t\t\t\tduty_cycle_max = 0.0;\n";
verilog_tb "\t\t\t\tend\n";
verilog_tb "\t\t\telse\n";
verilog_tb "\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\tif(output_50 == 1'b1)\n";
verilog_tb "\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\tif (last_falling > 0)\n";
verilog_tb "\t\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\t\tpw_low = \$time - last_falling;\n";
verilog_tb "\t\t\t\t\t\t\t\t\ttotal_time_low = total_time_low + pw_low;\n";
verilog_tb "\t\t\t\t\t\t\t\t\tmeasured_duty_cycle_avg <= (total_time_high/`clock_half_period)/((total_time_high + total_time_low)/`clock_half_period);\n";
verilog_tb "\t\t\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\t\t\tlast_rising = \$time;\n";
verilog_tb "\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\tif(output_50 == 1'b0)\n";
verilog_tb "\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\tif (last_rising > 0)\n";
verilog_tb "\t\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\t\tpw_high = \$time - last_rising;\n";
verilog_tb "\t\t\t\t\t\t\t\t\ttotal_time_high = total_time_high + pw_high;\n";
verilog_tb "\t\t\t\t\t\t\t\t\tif (pw_low > 0)\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\tduty_cycle = (pw_high/`clock_half_period)/((pw_high + pw_low)/`clock_half_period);\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\tif (duty_cycle < duty_cycle_min)\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\tduty_cycle_min = duty_cycle;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\tmeasured_duty_cycle_min <= duty_cycle_min;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\tif (duty_cycle > duty_cycle_max)\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\tduty_cycle_max = duty_cycle;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\t\tmeasured_duty_cycle_max <= duty_cycle_max;\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\t\t\tlast_falling = \$time;\n";
verilog_tb "\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\tend\n";
verilog_tb "\t\tend\n";
verilog_tb "\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\t// Process to check that output_50 and output_pulse are the same frequency.\n";
verilog_tb "\t// (to check that the duty cycle correction doesn't miss pulses, glitch, etc.)\n";
verilog_tb "\t// Comparison is disabled when output_50 is not driven, as is the case when\n";
verilog_tb "\t// fin/fout <= 2.  (VHDL gives 'U' and Verilog gives 'Z')\n";
verilog_tb "\t// There is no explicit test; the error is detected when the variable\n";
verilog_tb "\t// 'phase_difference' goes out of range.\n";
verilog_tb "\t// Note that certain versions of Modelsim have the range checking turned off\n";
verilog_tb "\t// by default.\n";
verilog_tb "\t";
comment_line_tb;
verilog_tb "\n";
verilog_tb "\talways @(posedge async_reset or posedge output_50 or posedge clock)\n";
verilog_tb "\t\tbegin: check_output_phase\n";
verilog_tb "\t\t\treg [1:0] phase_difference;\n";
verilog_tb "\t\t\tif (async_reset == 1'b1)\n";
verilog_tb "\t\t\t\tphase_difference = 0;\n";
verilog_tb "\t\t\telse\n";
verilog_tb "\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\tif(clock == 1'b1)\n";
verilog_tb "\t\t\t\t\t\tbegin\n";
verilog_tb "\t\t\t\t\t\t\tif (output_pulse === 1'b1 && output_50 !== 1'bz && output_50 !== 1'bz)\n";
verilog_tb "\t\t\t\t\t\t\t\tphase_difference = phase_difference + 1;\n";
verilog_tb "\t\t\t\t\t\tend\n";
verilog_tb "\t\t\t\t\tif(output_50 == 1'b1)\n";
verilog_tb "\t\t\t\t\t\tphase_difference = phase_difference - 1;\n";
verilog_tb "\t\t\t\tend\n";
verilog_tb "\t\tend\n";
verilog_tb "\n";
verilog_tb "endmodule\n";
verilog_tb "\n";
comment_line_tb;
comment_tb " <EOF> $verilog_tb_name\n";
comment_line_tb;
